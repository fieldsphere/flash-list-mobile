---
name: ios-simulator
description: Use when validating FlashList fixture behavior, checking simulator UI state, capturing visual proof, or debugging iOS screen interactions after the app is runnable by bundle ID.
...
```bash
source .cursor/skills/ios-simulator/scripts/setup-session.sh
```

Open the app:

```bash
.cursor/skills/ios-simulator/scripts/open-app.sh
```
...
```bash
.cursor/skills/ios-simulator/scripts/capture-screen.sh screen
```
...
.cursor/skills/ios-simulator/scripts/capture-screen.sh after
```
